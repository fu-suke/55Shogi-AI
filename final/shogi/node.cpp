...
double Node::search(double beta) {
    ...
    if (depth == MAX_DEPTH) {
        ...
        this->score = eval_playout_score(~pos.side_to_move);
        // this->score = calc_playout_score();
    ...
    // 今ここをいじるのは怖いので、一旦コメントアウト
    // 合法手がない（＝詰み）の場合は前の手番側の勝ち
    // より浅い詰みを選ぶために深さで割る
    // if (alpha == -INFTY) {
    //     this->score = -INFTY / this->depth;
    //     return this->score;
...
double Node::playout() {
    ...
    return eval_pieces(color_us);
}

double Node::calc_playout_score() {
    const int LOOP = 1000;
...
int Node::load_model() {
    ...
    std::string model_name = "model4_100_0113_1730.pt";
    // std::string model_name = "model2_0109_1221_cpu.pt";
    if (!is_model_loaded) {
        try {
            // デスクトップPCからの実行はこっち
            model = torch::jit::load(
                "H:\\マイドライブ\\Tsukuba\\2023春学期\\Special-"
                "Seminar\\Shogi\\final\\shogi\\" +
                model_name);
            model.eval();
        ...
        // try {
        //     // PetitShogiから呼び出すときはこっち
        //     model = torch::jit::load("../../shogi/" + model_name);
        //     model.eval();
        //     is_model_loaded = true;
        //     std::cout << "Model loaded from ../../shogi/" + model_name
        //               << std::endl;
        //     return 0;
        // } catch (const c10::Error &) {
        //     ;
